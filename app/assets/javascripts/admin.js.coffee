...
$ ->
  ...
  $('.choose_textures').on 'click', ->
    th = $(this)
    texture = th.attr("data-texture-id")
    $.ajax
      ...
      success: (data) ->
        $('body').append(data)
        $('#popup').fadeIn(200)
        $('.choose_texture').click ->
          th.prev('input').val($(this).attr("data-id"))
          th.text($(this).find('span').text())
          $('#popup').fadeOut 200, ->
            $('#popup').remove()